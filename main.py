...
# ETHNICITY: TURKISH (AZERI) | LOCATION: IRAN
# VERSION: 11.5.0 (GLOBAL SERVER EDITION)
# ============================================================
...
SETTINGS = {
    "TOKEN": "8449240979:AAFHcj8leRIVrHpMxZbq8g6oWvvdDUe4YS0",
...
# --- [ بخش ۲: مغز هوشمند (شامل بینایی و متن) ] ---
class OmniBrain:
    @staticmethod
...
class OmniInterface:
    ...
    async def start_action(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        # شروع همیشه انگلیسی برای کلاس جهانی
        welcome_text = (
    ...
    async def button_logic(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if query.data == 'verify_ok':
            ...
            confirm = "🔥 **System Active.** Send text or photo." if lang == "en" else "🔥 **سیستم فعال شد.** متن بفرست یا عکس آپلود کن."
    ...
    async def text_logic(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not db.check_verification(user_id):
    ...
    async def photo_logic(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        wait_txt = "👁 **Analyzing image...**" if lang == 'en' else "👁 **در حال آنالیز تصویر...**"
        waiting_msg = await update.message.reply_text(wait_txt)
        ...
        try:
            ...
            if response:
                ...
                await waiting_msg.edit_text("❌ Connection Error.")
        except:
            await waiting_msg.edit_text("❌ Technical Error.")
